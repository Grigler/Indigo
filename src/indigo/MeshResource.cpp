  ...
  if (inputFile.is_open())
  {
    ...
    while (std::getline(inputFile, currentLine))
    {
      ...
      else if (!currentLine.substr(0, 2).compare(0, 1, "f"))
      {
        ...
        {
          Application::ErrPrint("WARNING: This OBJ loader only works with triangles but a quad has been detected. Please triangulate your mesh.");
          inputFile.close();
          return;
        }
    ...
    vertCount = orderedPositionData.size();
    ...
    if (vertCount > 0)
    {
      ...
      glBufferData(GL_ARRAY_BUFFER, sizeof(float) * vertCount * 3, &orderedPositionData[0], GL_STATIC_DRAW);
      ...
      if (orderedNormalData.size() > 0)
      {
        ...
        // We need to tell it how much data to send
        // We can also tell OpenGL how we intend to use this buffer - here we say GL_STATIC_DRAW because we're only writing it once
        glBufferData(GL_ARRAY_BUFFER, sizeof(float) * vertCount * 3, &orderedNormalData[0], GL_STATIC_DRAW);
      ...
      if (orderedUVData.size() > 0)
      {
        ...
        // We need to tell it how much data to send
        // We can also tell OpenGL how we intend to use this buffer - here we say GL_STATIC_DRAW because we're only writing it once
        glBufferData(GL_ARRAY_BUFFER, sizeof(float) * vertCount * 2, &orderedUVData[0], GL_STATIC_DRAW);
